...
def show_shapes(img, conv_op, sigmoid_op, avg_pool_op, max_pool_op):
    print("""
         ...
         """.format(img.shape, conv_op.shape, sigmoid_op.shape, avg_pool_op.shape, max_pool_op.shape))
...
def convolve(img, kernel, strides=[1, 3, 3, 1], pooling=[1, 3, 3, 1], padding='SAME', rgb=True):
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            session.run(init)
            conv_op, sigmoid_op, avg_pool_op, max_pool_op = session.run([conv, sig, avg_pool, max_pool], feed_dict={x: img_reshaped})
...
if __name__ == '__main__':
    img = Image.open('d:/tmp/img/img_data/raw/Hydrangeas.jpg')
    
    k = np.ones([3, 3, 3, 3], dtype=np.float32) * -1
    k[1, 1, :, :] = 9 
