...
from cifar10dev import cifar10_input

FLAGS = tf.app.flags.FLAGS

# Basic model parameters.
tf.app.flags.DEFINE_integer('batch_size', 128, """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('data_dir', 'd://tmp//tensorflow//cifar10//cifar10_data', """Path to the CIFAR-10 data directory.""")
...
#         weights = _variable_with_weight_decay('weights', shape=[dim, 384], stddev=0.04, wd=0.004)
        ...
        weights = _variable_with_weight_decay('weights', shape=[reshape.shape[-1], 384], stddev=0.04, wd=0.004)
...
def loss(logits, labels):
    ...
    concated = tf.concat([indices, sparse_labels], 1)  # connect using column dimension [batch_size x 2]
    dense_labels = tf.sparse_to_dense(concated, [FLAGS.batch_size, NUM_CLASSES], 1.0, 0.0)  # [batch_size x NUM_CLASSES]
